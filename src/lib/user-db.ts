...
export type UserRole = 'super_admin' | 'events_manager' | 'news_manager' | 'user_manager';
...
export async function getUserById(id: string): Promise<DatabaseResult<User>> {
  try {
    const result = await usersDB.findOne(user => user.id === id && user.isActive);
    if (!result.success) {
      return result;
    }
...
// Get user by email
export async function getUserByEmail(email: string): Promise<DatabaseResult<User>> {
  try {
    const result = await usersDB.findOne(user => user.email === email && user.isActive);
    if (!result.success) {
      return result;
    }
...
// Create new user
export async function createUser(userData: CreateUserData): Promise<DatabaseResult<User>> {
  try {
...
// Update user
export async function updateUser(id: string, userData: UpdateUserData): Promise<DatabaseResult<User>> {
  try {
    // Check if new email already exists (exclude current user)
    if (userData.email !== undefined) {
      const existingUser = await usersDB.findOne(user => user.email === userData.email && user.id !== id);
      if (existingUser.success) {
        return { success: false, error: 'User with this email already exists' };
      }
    }
...
// Authenticate user
export async function authenticateUser(email: string, password: string): Promise<DatabaseResult<User>> {
  try {
    ...
    const isValidPassword = await bcrypt.compare(password, userResult.data.password);
    ...
    const { password: _, ...userResponse } = userResult.data;
...
// Get users by role
export async function getUsersByRole(role: UserRole): Promise<DatabaseResult<User[]>> {
  try {
    const result = await usersDB.findWhere(user => user.role === role && user.isActive);
    if (!result.success) {
      return result;
    }
