...
async function getNewsData(): Promise<NewsArticle[]> {
  if (typeof window !== "undefined") {
    throw new Error(
      "News database operations can only be performed on the server side"
    );
  }
...
export async function getAllNews(
  limit?: number
): Promise<DatabaseResult<NewsArticle[]>> {
  try {
...
export async function createNews(
  newsData: Omit<NewsArticle, "id" | "created_at" | "updated_at" | "views">
): Promise<DatabaseResult<NewsArticle>> {
  try {
    const news = await getNewsData();
