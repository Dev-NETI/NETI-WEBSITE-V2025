...
export async function GET(
  ...
  try {
    ...
    // Check permissions
    if (admin.role !== 'super_admin' && admin.role !== 'user_manager' && admin.id !== params.id) {
      return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    const canUpdate = admin.role === 'super_admin' || 
    ...
    if ((role !== undefined || isActive !== undefined) && 
        admin.role !== 'super_admin' && admin.role !== 'user_manager') {
      return NextResponse.json(
        { success: false, error: 'Insufficient permissions to change role or status' },
        { status: 403 }
      );
    }
    ...
    if (role === 'super_admin' && admin.role !== 'super_admin') {
      return NextResponse.json(
        { success: false, error: 'Only super administrators can assign super admin role' },
        { status: 403 }
      );
    }
